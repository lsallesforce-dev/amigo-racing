...
export async function qrCodeProxyHandler(req: Request, res: Response) {
  try {
    ...
    if (!url || typeof url !== 'string') {
      console.error('[QRCodeProxy] URL não fornecida ou inválida');
      return res.status(400).json({ error: 'URL não fornecida' });
    }

    // Validar que é uma URL do Pagar.me/Stone
    if (!url.includes('stone.com.br') && !url.includes('pagarme') && !url.includes('pix')) {
      console.error('[QRCodeProxy] URL não permitida:', url);
    ...
    console.error('[QRCodeProxy] Erro ao processar requisição:', error);
