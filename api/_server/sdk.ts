...
class OAuthService {
  ...
  async getUserInfoByToken(
    ...
    console.log('[OAuth] Chamando GetUserInfo com accessToken');
    ...
    console.log('[OAuth] GetUserInfo retornou com sucesso');
    return data;
  }
}
...
class SDKServer {
  ...
  async verifySession(
    ...
    try {
      const secretKey = this.getSessionSecret();
      console.log("[Auth] Verifying session with secret length:", ENV.cookieSecret.length);
      ...
      console.warn("[Auth] Session verification failed!", {
        error: String(error),
        cookiePreview: cookieValue.substring(0, 10) + "..."
      });
      return null;
    }
  }
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    console.log("[Auth] Validating session.", {
      ...
      hasReqCookies: !!(req as any).cookies,
    ...
    if (!session) {
      console.log("[Auth] Session validation failed or missing.");
